...
// Hitung total seluruh murid untuk persentase
$qTotalMurid = mysqli_query($conn, "SELECT COUNT(*) AS total FROM murid");
$totalMurid = (int) mysqli_fetch_assoc($qTotalMurid)['total'];
...
// Pemasukan per bulan (Menghitung berapa murid yang membayar)
$qMasukKat = mysqli_query($conn, "
    SELECT 
        bulan AS kategori, 
        SUM(jumlah) AS total_uang, 
        COUNT(DISTINCT nisn) AS jumlah_murid_bayar
    FROM transaksi
...
");
$dataDetailMasuk = [];
while ($r = mysqli_fetch_assoc($qDetailMasuk))
    $dataDetailMasuk[] = $r;
...
");
$dataDetailKeluar = [];
while ($r = mysqli_fetch_assoc($qDetailKeluar))
    $dataDetailKeluar[] = $r;
...
<head>
    ...
    <link rel="stylesheet" href="../../assets/css/ketua_kelas/laporan_kas.css">
</head>
    ...
    <div class="main-content">
        ...
        <div class="kategori-grid fade-in">
            ...
            <div class="kategori-card fade-in">
                <h5 class="kategori-title">Persentase Pemasukan Per Bulan</h5>
                ...
                <?php else: ?>
                    <?php foreach ($dataMasukKat as $item):
                        // Rumus : (Murid yang bayar / Total Murid) * 100
                        $pct = $totalMurid > 0 ? round(($item['jumlah_murid_bayar'] / $totalMurid) * 100) : 0;
                        ?>
                        <div class="kategori-item">
                            <div class="kategori-row">
                                <span class="kategori-nama">
                                    <?= htmlspecialchars($item['kategori']) ?>
                                    <small class="text-secondary">(<?= $item['jumlah_murid_bayar'] ?>/<?= $totalMurid ?>
                                        Murid)</small>
                                </span>
                                <span class="kategori-nominal"><?= rupiah($item['total_uang']) ?></span>
                            </div>
                            <div class="kategori-bar-wrap">
                                <div class="kategori-bar bar-masuk" style="width: <?= $pct ?>%"></div>
                            </div>
                            <div class="kategori-pct"><?= $pct ?>% Siswa Bayar</div>
                        </div>
                    <?php endforeach; ?>
                <?php endif; ?>
            </div>
                    ...
                    <table class="detail-table">
                        ...
                        <tbody>
                            <?php if (empty($dataDetailMasuk)): ?>
                                <tr>
                                    <td colspan="5" style="text-align:center;font-style:italic;color:#94a3b8;padding:32px">Belum
                                        ada data</td>
                                </tr>
                            <?php else: ?>
